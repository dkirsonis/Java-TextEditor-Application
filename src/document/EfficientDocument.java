...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for(String word : tokens){						//for each element in tokens
			if(isWord(word)){							//if isWord returns true
				numWords++;								//increment words
				numSyllables += countSyllables(word);	//count syllables in the word
			}
			else
				numSentences++;							//isWord returned false, so its a sentence not a word
		}
		if(tokens.size() != 0){							//if not an empty text
			String lastString = tokens.get(tokens.size()-1);			//get the last entry in tokens
			char lastChar = lastString.charAt(lastString.length()-1); 	//get the last char in the last entry
			if(lastChar != '.' && lastChar != '!' && lastChar != '?')	//if the text doesn't end with punctuation
				numSentences++;
		}
	}
